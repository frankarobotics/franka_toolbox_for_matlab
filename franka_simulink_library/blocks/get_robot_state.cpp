...
#define S_FUNCTION_NAME get_robot_state
...
static void mdlInitializeSizes(SimStruct* S) {
  ...
  // Parse the sfunction parameters(total 1)

  // 1. output_signals_str (and figure out which and how many output signals are requested)
  size_t output_singals_str_length = mxGetNumberOfElements(ssGetSFcnParam(S, 0));
  char* output_singals_str = static_cast<char*>(mxCalloc(output_singals_str_length, sizeof(char)));
  mxGetString(ssGetSFcnParam(S, 0), output_singals_str, output_singals_str_length + 1);

  std::vector<std::string> output_signals_robot_state_;
  
  signalsStringToSignalsVector(output_signals_robot_state_, output_singals_str);
  ...
  if (!ssSetNumOutputPorts(S, output_signals_robot_state_.size()))
    return;
  for (int i = 0; i < output_signals_robot_state_.size(); i++) {
    ssSetOutputPortWidth(S, i, getWidthForSignalString[output_signals_robot_state_[i]]);
...
{
    size_t output_singals_str_length = mxGetNumberOfElements(ssGetSFcnParam(S, 0));
    char* output_singals_str = (char *)mxCalloc(output_singals_str_length, sizeof(char));
    mxGetString(ssGetSFcnParam(S, 0), output_singals_str, output_singals_str_length + 1);
    
    size_t robot_ip_length = mxGetNumberOfElements(ssGetSFcnParam(S, 1)) + 1;
    static char robot_ip_[20];
    mxGetString(ssGetSFcnParam(S, 1), robot_ip_, robot_ip_length);
    ...
    if ( !ssWriteRTWParamSettings(S, 2,
            SSWRITE_VALUE_QSTR, "output_signals_str", output_singals_str,
